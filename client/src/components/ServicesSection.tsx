    ...
    <section id="services" className="py-32 relative" ref={containerRef}>
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <div className="mb-20 max-w-3xl">
          ...
          <h3 className="text-4xl md:text-5xl font-bold text-foreground mb-6">Digital Excellence <br/><span className="text-slate-400 dark:text-slate-500">Engineered</span></h3>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {displayServices.map((service) => {
            ...
            return (
              <div 
                key={service.id}
                className="service-card spotlight-wrapper h-full bg-slate-50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 rounded-2xl p-8 transition-transform duration-300 hover:-translate-y-2 magnetic-interactive"
                onMouseMove={handleMouseMove}
              >
                <div className="relative z-10">
                  <div className="w-14 h-14 rounded-xl bg-white dark:bg-white/5 flex items-center justify-center mb-6 border border-slate-200 dark:border-white/10 shadow-sm dark:shadow-none">
                  ...
                  <p className="text-slate-600 dark:text-slate-400 leading-relaxed font-sans">{service.description}</p>
                </div>
              </div>
            );
          })}
